...
class ComentarioInputComentario_Dentro extends StatefulWidget {
  final dynamic lista;
  final Function(dynamic) onComentarioAgregado;
...
class _ComentarioInputState extends State<ComentarioInputComentario_Dentro> {
  final dynamic lista;
  ...
  void _enviarComentario() async {
    ...
    String serieId =
        lista['serieId']; // Obtén el ID de la serie desde el widget
    String capitulo = lista['capituloId'];
    ...
    await seriesService.AgregarComentario(
        comentario, usuarioId, serieId, capitulo, userName, token);
